    ...
    public class ClassGlobales
    {
        ...
        public void EjecutarScriptSQL(string vNombreArchivoSQL)
        {
            ...
            try
            {
                ...
                GenerarLOG_DB("SCIRPT", sqlText, "CORRECTO", "NINGUNO");
            }
            catch (Exception EX)
            {
                string vSQLScript = Application.StartupPath + "\\SCRIPTS\\" + vNombreArchivoSQL;
                ...
                GenerarLOG_DB("SCIRPT", sqlText, "ERRONEO", EX.ToString());
            }
        ...
        public static void GenerarLOG_DB(String vOPERACION, String vVALOR, String vESTATUS, String vMENSAJE_ERROR)
        {
            ...
            vEventos[0] = String.Format("Tiempo inicio aplicación: {0}", DateTime.Now.ToString());
            ...
            string vFechaFolder = DateTime.Now.ToString("dddd") + " " + DateTime.Now.ToString("dd") + " de " + DateTime.Now.ToString("MMMM") + " del " + DateTime.Now.ToString("yyyy");

            //SE COMPARA SI EL ERROR ES DIFERENTE A NINGUNO 
            if (vMENSAJE_ERROR == "NINGUNO")
            {
                vArchivo = FileSystem.CurDir() + "\\LOGS\\GENERALES\\" + vFechaFolder + "\\ LOG generado el día " + vFecha + ".txt";

            }
            else
            {
                vArchivo = FileSystem.CurDir() + "\\LOGS\\ERROR\\" + vFechaFolder + "\\ LOG generado el día " + vFecha + ".txt";
            }
            ...
            string Folder = Application.StartupPath + "\\LOGS\\GENERALES\\" + vFechaFolder;

            string FolderError = Application.StartupPath + "\\LOGS\\ERROR\\" + vFechaFolder;


            try
            {
                if (!System.IO.Directory.Exists(Application.StartupPath + "\\LOGS\\GENERALES\\" + vFechaFolder))
                {
                    //La carpeta no existe, Se debe de crear
                    System.IO.Directory.CreateDirectory(Folder);
                ...
                if (!System.IO.Directory.Exists(Application.StartupPath + "\\LOGS\\ERROR\\" + vFechaFolder))
                {
                    //La carpeta no existe, Se debe de crear
                    System.IO.Directory.CreateDirectory(FolderError);
